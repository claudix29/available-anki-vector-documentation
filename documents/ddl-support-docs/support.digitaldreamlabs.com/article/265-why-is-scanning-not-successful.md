::: {#mainNav .navbar}
::: navbar-inner
::: container-fluid
[![Digital Dream Labs Knowledge
Base](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/63222136c713d51da3ede832/logo-75h.png){width="195"
height="75"}](https://support.digitaldreamlabs.com/){.brand}

[Toggle Navigation]{.sr-only} []{.icon-bar} []{.icon-bar} []{.icon-bar}

::: {.nav-collapse .collapse}
-   [[Home ](/)]{#home}
-   [[General Information
    ](/collection/1-general-information)]{#general-information}
-   [[Vector ](/collection/6-vector)]{#vector}
-   [[Cozmo ](/collection/9-cozmo)]{#cozmo}
-   [[OverDrive ](/collection/62-overdrive)]{#overdrive}
-   [[Escape Pod ](/collection/364-escape-pod)]{#escape-pod}
-   [[Puzzlets ](/collection/325-puzzlets)]{#puzzlets}
-   [[Contact](#){.contactUs onclick="window.Beacon('open')"}]{#contact}
-   [[Contact](#){.contactUs
    onclick="window.Beacon('open')"}]{#contactMobile}
:::
:::
:::
:::

::: {#contentArea .section .container-fluid}
::: row-fluid
::: {#main-content .section .span9}
::: contentWrapper
# Why is scanning not successful? {#why-is-scanning-not-successful .title}

[](javascript:window.print() "Print this article"){.printArticle
aria-label="Print this Article"}

This FAQ lists several potential issues you may see when scanning a
track and how to overcome those issues.

------------------------------------------------------------------------

::: sections
## Sections

**[Issues you may be experiencing](#h_443456486241531750335594) \|\| 
[Normal behavior](#h_844006822661531750341824) \|\|  [Troubleshooting
the basics](#h_6378507781101531750352066) \|\| [Further
troubleshooting](#h_1409385651421531750358625)**
:::

------------------------------------------------------------------------

::: sections
## Issues you may be experiencing

-   Vehicles do not start scanning
-   Vehicles do not line up properly
-   Vehicles do not stop scanning
-   Incorrect track configuration displayed in host app
-   Vehicle(s) do not appear in the host's app 
:::

------------------------------------------------------------------------

::: sections
## Normal behavior

-   Larger vehicles (BigBang, Supertrucks) struggle to complete scan,
    especially if the track includes a Launch Kit, Bank Turn Kit or
    Elevation Kit. The vehicle may stall, drive off track or do a
    wheelie

     
  ![trucks.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d775304286364bc975cb4/img-4276-1585280679-667922747.png){width="169"
height="190"} ![no_trucks.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d775404286364bc975cb5/img-4276-1585280679-387098095.png){width="421"
height="190"}
:::

------------------------------------------------------------------------

::: sections
## Troubleshooting the basics

-   check you are using a [compatible
    device](//support.digitaldreamlabs.com/article/255-compatible-devices-anki-overdrive-fast-furious-edition)
-   check you have built a [valid track
    configuration](//support.digitaldreamlabs.com/article/272-how-to-build-the-best-tracks). While
    troubleshooting, use a simple oval track - \"Microloop\" - with 1
    start/finish track piece, 1 straight track piece and 4 curved track
    pieces
-   Check the rear tires are not misaligned - the rim of the wheels
    should not be visible

     
 ![Screenshot_2018-07-16_15.18.51.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d77572c7d3a7e9ae9e3e5/img-4276-1585280680-1115634302.png){width="254"
height="179"}

![](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d76ac04286364bc975bb4/img-4276-1585280680-1496185564.gif)

    
:::

------------------------------------------------------------------------

::: sections
## Further troubleshooting

-   move the vehicles slightly forward or backward to line them up
    before you begin the game. This will not alter the results or the
    start of the game in any way
-   check you see all connected vehicles in the host app after scanning
    begins

       
 ![power_zone_scan_2.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d77572c7d3a7e9ae9e3e7/img-4276-1585280680-612648108.png){width="394"
height="229"}

-   try placing the vehicles on the track the wrong way around. Once
    scanning starts, they should correct their direction after
    approximately 1 lap
-   log into a different profile
:::

::: {.articleRatings article-url="https://support.digitaldreamlabs.com/article/265-why-is-scanning-not-successful"}
[Did this answer your question?]{.articleRatings-question} [ [
![](data:image/svg+xml;base64,)
]{.articleRatings-feedback-tick} [Thanks for the
feedback]{.articleRatings-feedback-message} ]{.articleRatings-feedback}
[There was a problem submitting your feedback. Please try again
later.]{.articleRatings-failure}

::: articleRatings-actions
[Yes]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}

[No]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}
:::
:::

::: {.section .articleFoot}

Still need help? [Contact Us](#){#sbContact .contactUs
onclick="window.Beacon('open')"} [Contact Us](#){#sbContactMobile
.contactUs onclick="window.Beacon('open')"}

Last updated on January 28, 2022
:::
:::
:::

[Toggle Search]{.sr-only}

::: {#serp-dd .sb style="display: none;"}
:::

### Categories

-   [Documents & Manuals ](/category/511-documents-manuals)
-   [Frequently Asked Questions
    ](/category/508-frequently-asked-questions)
-   [Legacy Products ](/category/506-legacy-products)
-   [OverDrive 2.6 Relaunched ](/category/505-overdrive-26-relaunched)
-   [Track & Car Maintenance ](/category/507-track-car-maintenance)
-   [Track Tips ](/category/509-track-tips)
-   [Technical Troubleshooting
    ](/category/328-technical-issues-troubleshooting)
:::

::: {#noResults style="display:none;"}
No results found
:::

© [Digital Dream Labs](https://support.digitaldreamlabs.com/) 2023.
Powered by [Help
Scout](https://www.helpscout.com/knowledge-base/?utm_source=docs&utm_medium=footerlink&utm_campaign=Docs+Branding){rel="nofollow noopener noreferrer"
target="_blank"}
:::
