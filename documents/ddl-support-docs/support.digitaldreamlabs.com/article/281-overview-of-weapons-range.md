::: {#mainNav .navbar}
::: navbar-inner
::: container-fluid
[![Digital Dream Labs Knowledge
Base](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/63222136c713d51da3ede832/logo-75h.png){width="195"
height="75"}](https://support.digitaldreamlabs.com/){.brand}

[Toggle Navigation]{.sr-only} []{.icon-bar} []{.icon-bar} []{.icon-bar}

::: {.nav-collapse .collapse}
-   [[Home ](/)]{#home}
-   [[General Information
    ](/collection/1-general-information)]{#general-information}
-   [[Vector ](/collection/6-vector)]{#vector}
-   [[Cozmo ](/collection/9-cozmo)]{#cozmo}
-   [[OverDrive ](/collection/62-overdrive)]{#overdrive}
-   [[Escape Pod ](/collection/364-escape-pod)]{#escape-pod}
-   [[Puzzlets ](/collection/325-puzzlets)]{#puzzlets}
-   [[Contact](#){.contactUs onclick="window.Beacon('open')"}]{#contact}
-   [[Contact](#){.contactUs
    onclick="window.Beacon('open')"}]{#contactMobile}
:::
:::
:::
:::

::: {#contentArea .section .container-fluid}
::: row-fluid
::: {#main-content .section .span9}
::: contentWrapper
# Overview of Weapons\' Range {#overview-of-weapons-range .title}

[](javascript:window.print() "Print this article"){.printArticle
aria-label="Print this Article"}

There are 5 ranges of weapons.

------------------------------------------------------------------------

::: sections
## [ Short Range weapons]{.wysiwyg-font-size-large}

![](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d76c304286364bc975bd8/img-25345-1585280704-938540465.gif)

-   **Strengths:** A wide blast radius
-   **Weaknesses:** Limited reach 
-   **Recommendations:** It\'s best to use them when your target is
    close, like going into a turn
:::

------------------------------------------------------------------------

::: sections
## [Long Range weapons]{.wysiwyg-font-size-large}

![](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d76c304286364bc975bd9/img-25345-1585280705-1720936679.gif)

-   **Strengths:** An extended reach
-   ****Weaknesses:**** A narrow blast radius
-   **Recommendations: **Make sure you\'re lined up on a straightaway
    for the best results
:::

------------------------------------------------------------------------

::: sections
## [Medium Range weapons]{.wysiwyg-font-size-large}

![](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d76c304286364bc975bda/img-25345-1585280705-1382185410.gif)

-   ****Strengths:**** A strong balance between reach and blast radius
-   **Weaknesses:** Not as exciting as some other weapons
-   **Recommendations:** When in doubt, take the shot
:::

------------------------------------------------------------------------

::: sections
## [Deployed weapons]{.wysiwyg-font-size-large}

![](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d76c404286364bc975bdb/img-25345-1585280705-139364508.gif)

-   **Strengths:** An explosive that is dropped from behind your
    Supercar that you don\'t have aim
-   **Weaknesses:** It doesn\'t move and your target has to be close
    behind you
-   **Recommendations:** Set it and forget it
:::

------------------------------------------------------------------------

::: sections
## [Area of Effect weapons]{.wysiwyg-font-size-large}

![](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d76c42c7d3a7e9ae9e2f3/img-25345-1585280706-1744694077.gif)

-   ****Strengths:**** A powerful blasts that radiate around your
    supercar, so there\'s no need to aim
-   **Weaknesses:** You have to be close to your target
-   **Recommendations:** If you\'re neck and neck, fire away
:::

------------------------------------------------------------------------

::: sections
## Feel free to watch Under The Hood - Weapon Types
:::

::: {.articleRatings article-url="https://support.digitaldreamlabs.com/article/281-overview-of-weapons-range"}
[Did this answer your question?]{.articleRatings-question} [ [
![](data:image/svg+xml;base64,)
]{.articleRatings-feedback-tick} [Thanks for the
feedback]{.articleRatings-feedback-message} ]{.articleRatings-feedback}
[There was a problem submitting your feedback. Please try again
later.]{.articleRatings-failure}

::: articleRatings-actions
[Yes]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}

[No]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}
:::
:::

::: {.section .articleFoot}

Still need help? [Contact Us](#){#sbContact .contactUs
onclick="window.Beacon('open')"} [Contact Us](#){#sbContactMobile
.contactUs onclick="window.Beacon('open')"}

Last updated on January 28, 2022
:::
:::
:::

[Toggle Search]{.sr-only}

::: {#serp-dd .sb style="display: none;"}
:::

### Categories

-   [Documents & Manuals ](/category/511-documents-manuals)
-   [Frequently Asked Questions
    ](/category/508-frequently-asked-questions)
-   [Legacy Products ](/category/506-legacy-products)
-   [OverDrive 2.6 Relaunched ](/category/505-overdrive-26-relaunched)
-   [Track & Car Maintenance ](/category/507-track-car-maintenance)
-   [Track Tips ](/category/509-track-tips)
-   [Technical Troubleshooting
    ](/category/328-technical-issues-troubleshooting)
:::

::: {#noResults style="display:none;"}
No results found
:::

© [Digital Dream Labs](https://support.digitaldreamlabs.com/) 2023.
Powered by [Help
Scout](https://www.helpscout.com/knowledge-base/?utm_source=docs&utm_medium=footerlink&utm_campaign=Docs+Branding){rel="nofollow noopener noreferrer"
target="_blank"}
:::
