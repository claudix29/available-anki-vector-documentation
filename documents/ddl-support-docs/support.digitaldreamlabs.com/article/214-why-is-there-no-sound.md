::: {#mainNav .navbar}
::: navbar-inner
::: container-fluid
[![Digital Dream Labs Knowledge
Base](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/63222136c713d51da3ede832/logo-75h.png){width="195"
height="75"}](https://support.digitaldreamlabs.com/){.brand}

[Toggle Navigation]{.sr-only} []{.icon-bar} []{.icon-bar} []{.icon-bar}

::: {.nav-collapse .collapse}
-   [[Home ](/)]{#home}
-   [[General Information
    ](/collection/1-general-information)]{#general-information}
-   [[Vector ](/collection/6-vector)]{#vector}
-   [[Cozmo ](/collection/9-cozmo)]{#cozmo}
-   [[OverDrive ](/collection/62-overdrive)]{#overdrive}
-   [[Escape Pod ](/collection/364-escape-pod)]{#escape-pod}
-   [[Puzzlets ](/collection/325-puzzlets)]{#puzzlets}
-   [[Contact](#){.contactUs onclick="window.Beacon('open')"}]{#contact}
-   [[Contact](#){.contactUs
    onclick="window.Beacon('open')"}]{#contactMobile}
:::
:::
:::
:::

::: {#contentArea .section .container-fluid}
::: row-fluid
::: {#main-content .section .span9}
::: contentWrapper
# Why is there no sound? {#why-is-there-no-sound .title}

[](javascript:window.print() "Print this article"){.printArticle
aria-label="Print this Article"}

This articles explains what to do if you\'re not hearing any sounds from
Cozmo.

![Cozmo_Volume_icon_0.5x.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d77472c7d3a7e9ae9e3d3/img-2828-1585280646-628874032.png)![Cozmo_Music_icon_0.5x.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d77472c7d3a7e9ae9e3d4/img-2828-1585280646-1667681242.png)

------------------------------------------------------------------------

::: sections
When you\'re interacting with Cozmo, there are two place from which you
can expect to hear sounds:

1.  [Your device](#h_68701992071543934287967)
2.  [Cozmo himself](#h_487842213111543934292638)

**Note:** applies only if Cozmo is not **muted **in Settings \> Volume &
General.
:::

------------------------------------------------------------------------

::: sections
## Your device {#h_68701992071543934287967}

If you\'re not hearing music or game cues (things like narration during
intro videos, prompts to play games, Cozmo detecting a cube, and the
notes played by Cubes during Memory Match), check the Volume settings on
your device (Volume Up/Down buttons) **AND** the Music level in Settings
\> Volume & General:

![Volume_settings_device_overlay.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d774704286364bc975ca5/img-2828-1585280646-2071685320.png)

You also should take a look at the device itself to see if it has an
issue and ensure the app is installed on internal memory.

[If you hear no sound or distorted sound from your iPhone, iPad, or iPod
touch speaker](https://support.apple.com/en-us/HT203794){target="_blank"
rel="noopener noreferrer"} \[support.apple.com\]

[FIX- How to fix no sound coming out from android
phone](https://www.lifewire.com/fix-android-phone-speaker-not-working-4580298){target="_blank"
rel="noopener noreferrer"} \[lifewire.com\]
:::

------------------------------------------------------------------------

::: sections
## Cozmo himself {#h_487842213111543934292638}

If you\'re not hearing Cozmo\'s voice when he wakes up, snoring when he
goes to sleep, or when using Cozmo Says, try increasing his volume in
Settings \> Volume & General:

![Untitled.png](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/5e7d77482c7d3a7e9ae9e3d5/img-2828-1585280646-113734008.png){width="548"
height="406"}

If changing the level of Cozmo\'s voice does not change or increase the
sounds he\'s making, if you still don\'t hear any sounds coming from
Cozmo, please contact us so we can help you further. 
:::

::: {.articleRatings article-url="https://support.digitaldreamlabs.com/article/214-why-is-there-no-sound"}
[Did this answer your question?]{.articleRatings-question} [ [
![](data:image/svg+xml;base64,)
]{.articleRatings-feedback-tick} [Thanks for the
feedback]{.articleRatings-feedback-message} ]{.articleRatings-feedback}
[There was a problem submitting your feedback. Please try again
later.]{.articleRatings-failure}

::: articleRatings-actions
[Yes]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}

[No]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}
:::
:::

::: {.section .articleFoot}

Still need help? [Contact Us](#){#sbContact .contactUs
onclick="window.Beacon('open')"} [Contact Us](#){#sbContactMobile
.contactUs onclick="window.Beacon('open')"}

Last updated on January 7, 2022
:::
:::
:::

[Toggle Search]{.sr-only}

::: {#serp-dd .sb style="display: none;"}
:::

### Categories

-   [Frequently Asked Questions
    ](/category/18-frequently-asked-questions)
-   [Setting Cozmo Up ](/category/496-setting-cozmo-up)
-   [Charging & Power ](/category/492-charging-power)
-   [Cozmo Cubes & Accessories ](/category/499-cozmo-cubes-accessories)
-   [Tips & Tricks ](/category/19-tips-tricks)
-   [Reset Cozmo ](/category/494-reset-cozmo)
-   [Code Lab ](/category/495-code-lab)
-   [Education Mode ](/category/498-education-mode)
-   [SDK Information ](/category/35-sdk-information)
-   [Privacy & Security ](/category/503-privacy-security)
-   [Error Troubleshooting ](/category/502-error-troubleshooting)
-   [Sound Troubleshooting ](/category/501-sound-troubleshooting)
-   [Connection Troubleshooting
    ](/category/497-connection-troubleshooting)
-   [Documents & Manuals ](/category/493-documents-manuals)
:::

::: {#noResults style="display:none;"}
No results found
:::

© [Digital Dream Labs](https://support.digitaldreamlabs.com/) 2023.
Powered by [Help
Scout](https://www.helpscout.com/knowledge-base/?utm_source=docs&utm_medium=footerlink&utm_campaign=Docs+Branding){rel="nofollow noopener noreferrer"
target="_blank"}
:::
