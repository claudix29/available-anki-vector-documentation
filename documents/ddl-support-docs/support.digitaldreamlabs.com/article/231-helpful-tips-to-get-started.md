::: {#mainNav .navbar}
::: navbar-inner
::: container-fluid
[![Digital Dream Labs Knowledge
Base](//d33v4339jhl8k0.cloudfront.net/docs/assets/5e3f0b1e2c7d3a7e9ae777f5/images/63222136c713d51da3ede832/logo-75h.png){width="195"
height="75"}](https://support.digitaldreamlabs.com/){.brand}

[Toggle Navigation]{.sr-only} []{.icon-bar} []{.icon-bar} []{.icon-bar}

::: {.nav-collapse .collapse}
-   [[Home ](/)]{#home}
-   [[General Information
    ](/collection/1-general-information)]{#general-information}
-   [[Vector ](/collection/6-vector)]{#vector}
-   [[Cozmo ](/collection/9-cozmo)]{#cozmo}
-   [[OverDrive ](/collection/62-overdrive)]{#overdrive}
-   [[Escape Pod ](/collection/364-escape-pod)]{#escape-pod}
-   [[Puzzlets ](/collection/325-puzzlets)]{#puzzlets}
-   [[Contact](#){.contactUs onclick="window.Beacon('open')"}]{#contact}
-   [[Contact](#){.contactUs
    onclick="window.Beacon('open')"}]{#contactMobile}
:::
:::
:::
:::

::: {#contentArea .section .container-fluid}
::: row-fluid
::: {#main-content .section .span9}
::: contentWrapper
# Helpful Tips to Get Started {#helpful-tips-to-get-started .title}

[](javascript:window.print() "Print this article"){.printArticle
aria-label="Print this Article"}

 A few things to keep in mind as you get started with Cozmo 

------------------------------------------------------------------------

::: sections
## The Cozmo app is an integral part of the Cozmo experience

-   When you choose a [**compatible
    device**](//support.digitaldreamlabs.com/article/135-compatible-devices){target="_blank"}
    that\'s going to be used regularly with Cozmo, be prepared that your
    kids might be demanding your device all of the time.  
-   Cozmo is seriously fun, so **we\'re warning you to be prepared for
    the demand for your device**.
:::

------------------------------------------------------------------------

::: sections
## [Where to play with Cozmo](//support.digitaldreamlabs.com/article/236-where-to-play-with-cozmo){target="_blank" rel="noopener noreferrer"}

Cozmo may work on carpets, but he definitely [prefers smooth and flat
surfaces]{.wysiwyg-underline} that are not highly reflective. So you
should avoid surfaces like mirrors or glass-topped tables.
:::

------------------------------------------------------------------------

::: sections
## [How do I wake up and reconnect to Cozmo?](//support.digitaldreamlabs.com/article/233-how-do-i-wake-up-and-reconnect-to-cozmo){target="_blank" rel="noopener noreferrer"}

Going somewhere? Even if Cozmo is already fully charged, remember to
bring his charger.

Cozmo has his own WiFi Network, but Cozmo never connects to the
Internet, and only ever connects to your device. 

-   So please remember, when your device is connected to Cozmo,
    it will not be connected to the Internet via WiFi.
-   When you're finished playing with Cozmo, your device normally will
    reconnect to your usual WiFi network, but you can change networks
    manually if you like.

Cozmo and the Cozmo app will remember the progress you\'ve made bonding
with him. Whatever Cozmo learns is stored inside Cozmo and backed up
inside of the app, including unlocks, game difficulty, and every name
and feature set saved in Cozmo\'s memory.
:::

::: {.articleRatings article-url="https://support.digitaldreamlabs.com/article/231-helpful-tips-to-get-started"}
[Did this answer your question?]{.articleRatings-question} [ [
![](data:image/svg+xml;base64,)
]{.articleRatings-feedback-tick} [Thanks for the
feedback]{.articleRatings-feedback-message} ]{.articleRatings-feedback}
[There was a problem submitting your feedback. Please try again
later.]{.articleRatings-failure}

::: articleRatings-actions
[Yes]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}

[No]{.sr-only} [
![](data:image/svg+xml;base64,)
]{.rating-face}
:::
:::

::: {.section .articleFoot}

Still need help? [Contact Us](#){#sbContact .contactUs
onclick="window.Beacon('open')"} [Contact Us](#){#sbContactMobile
.contactUs onclick="window.Beacon('open')"}

Last updated on January 7, 2022
:::
:::
:::

[Toggle Search]{.sr-only}

::: {#serp-dd .sb style="display: none;"}
:::

### Categories

-   [Frequently Asked Questions
    ](/category/18-frequently-asked-questions)
-   [Setting Cozmo Up ](/category/496-setting-cozmo-up)
-   [Charging & Power ](/category/492-charging-power)
-   [Cozmo Cubes & Accessories ](/category/499-cozmo-cubes-accessories)
-   [Tips & Tricks ](/category/19-tips-tricks)
-   [Reset Cozmo ](/category/494-reset-cozmo)
-   [Code Lab ](/category/495-code-lab)
-   [Education Mode ](/category/498-education-mode)
-   [SDK Information ](/category/35-sdk-information)
-   [Privacy & Security ](/category/503-privacy-security)
-   [Error Troubleshooting ](/category/502-error-troubleshooting)
-   [Sound Troubleshooting ](/category/501-sound-troubleshooting)
-   [Connection Troubleshooting
    ](/category/497-connection-troubleshooting)
-   [Documents & Manuals ](/category/493-documents-manuals)
:::

::: {#noResults style="display:none;"}
No results found
:::

© [Digital Dream Labs](https://support.digitaldreamlabs.com/) 2023.
Powered by [Help
Scout](https://www.helpscout.com/knowledge-base/?utm_source=docs&utm_medium=footerlink&utm_campaign=Docs+Branding){rel="nofollow noopener noreferrer"
target="_blank"}
:::
